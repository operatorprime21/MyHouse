...
public class Stairs2 : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
            ...
            else if (Input.GetKey(KeyCode.A) && Input.GetKey(KeyCode.W) == false)
            {
                stairs.isTrigger = true;
                secondFloor.isTrigger = false;
            }
            else if (Input.GetKey(KeyCode.D) && Input.GetKey(KeyCode.W) == true)
            {
                stairs.isTrigger = false;
                secondFloor.isTrigger = true;
            }
            else if (Input.GetKey(KeyCode.A) && Input.GetKey(KeyCode.W) == true)
            {
                stairs.isTrigger = false;
                secondFloor.isTrigger = true;
            }
